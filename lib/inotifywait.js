  ...
  self.runProcess = function () {
    ...
    Lazy(self.inwp.stdout)
        ...
        .forEach(function (event) {
          ...
          } else if (event.type.indexOf('MOVED_FROM') != -1) {
          ...
          } else if (event.type.indexOf('CLOSE_NOWRITE') != -1) {
            delete self.currentEvents[event.file];
            return
          } else if (event.type.indexOf('CLOSE') != -1) { // and 'CLOSE_WRITE'
            if (self.currentEvents[event.file]) {
              if (self.currentEvents[event.file] == 'open_attributes') {
                self.emit('attributes', event.file, stats)  //change of time: e.g.: touch -d "4 hours ago" filename
                delete self.currentEvents[event.file]
                return
              } else {
                if (self.currentEvents[event.file] != 'move') {
